...
# --- GCP专用究极稳定版：通过元数据服务器和接口名获取所有公网IP ---
get_gcp_public_ips() {
    local ips=()
    ...
    for iface in $interfaces; do
        # 【重大修正】直接使用接口名(iface)查询，不再用索引号
        local public_ip=$(curl -s -H "Metadata-Flavor: Google" "http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/$(echo $iface)/access-configs/0/external-ip")
        
        # 增加对HTML错误的过滤，确保只处理IP地址
        if [[ -n "$public_ip" && ! "$public_ip" == *"DOCTYPE html"* ]]; then
            # 检查IP是否已在数组中，防止重复
            if ! [[ " ${ips[@]} " =~ " ${public_ip} " ]]; then
                ips+=("$public_ip")
            fi
        else
            echo "Warning: Could not get public IP for interface '$iface'. Response: $public_ip"
        fi
    done
...
# --- 函数部分 (其余函数保持不变，仅修复了防火墙循环的一个小bug) ---
